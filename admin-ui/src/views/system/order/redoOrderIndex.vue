<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="重做类型" prop="modelType">
        <el-select v-model="queryParams.modelType" placeholder="请选择订单类型">
          <el-option v-for="(item,index) in redoOrderTypeList" :key="index" :label="item.dictLabel" :value="item.dictValue">
          </el-option>
        </el-select>
      </el-form-item>
      <el-form-item>
    ...
    <el-table v-loading="loading" :data="orderList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="重做原因" align="center" prop="returnWhy">
        <template slot-scope="scope">
            {{scope.row.returnList[0].returnWhy}}
        </template>
      </el-table-column>
      <el-table-column label="重做时间" align="center" prop="returnTime">
        <template slot-scope="scope">
          {{scope.row.returnList[0].createTime}}
        </template>
      </el-table-column>
      <el-table-column label="重做类型" align="center" prop="status">
        <template slot-scope="scope">
          <span v-for="data in redoOrderTypeList" v-if="data.dictSort==scope.row.returnList[0].returnStatus">
            {{data.dictLabel}}
          </span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          >订单详情</el-button>
          <el-button v-if="scope.row.returnList[0].returnStatus==1"
            size="mini"
            type="text"
            icon="el-icon-edit"
            @click="handleOrderApproval(scope.row)"
            v-hasPermi="['system:order:edit']"
          >已收货，重做中</el-button>
          <el-button v-if="scope.row.returnList[0].returnStatus==2"
                     size="mini"
                     type="text"
                     icon="el-icon-edit"
                     @click="handleOrderApproval(scope.row)"
    ...
    <div v-if="form!=null">
      <el-dialog :title="title" :visible.sync="open" width="1000px" append-to-body >
        <el-descriptions title="订单信息" direction="vertical" :column="4" border>
          ...
          <el-descriptions-item label="订单名称">{{form.name}}</el-descriptions-item>
          ...
          <el-descriptions-item label="订单重做状态">
            {{redoOrderStatus}}
          </el-descriptions-item>
        </el-descriptions>
        ...
        <el-descriptions v-for="(data,index) in form.returnList" :title="getTitle(index)" direction="vertical" :column="4" border >
            <el-descriptions-item label="重做原因">{{data.returnWhy}}</el-descriptions-item>
            <el-descriptions-item label="重做详情">{{data.returnWhydesc}}</el-descriptions-item>
            <el-descriptions-item label="操作时间">{{data.createTime}}</el-descriptions-item>
            <el-descriptions-item label="退回物流号">{{data.returnLogisticssn}}</el-descriptions-item>
            <el-descriptions-item label="新收货地址">
              <el-popover
                placement="top-start"
                title="新收货地址"
                width="500"
                ...
                <el-descriptions title="收货信息" direction="vertical" :column="4" border>
                  <el-descriptions-item label="收货人">{{data.tbAddressBook.consignee}}</el-descriptions-item>
                  <el-descriptions-item label="联系方式">{{data.tbAddressBook.phone}}</el-descriptions-item>
                  <el-descriptions-item label="性别">{{data.tbAddressBook.sex}}</el-descriptions-item>
                  <el-descriptions-item label="收货地址">{{data.tbAddressBook.provinceName}}--{{data.tbAddressBook.cityName}}--{{data.tbAddressBook.districtName}}</el-descriptions-item>
                  <el-descriptions-item label="详细地址">{{data.tbAddressBook.detail}}</el-descriptions-item>
                </el-descriptions>
                <el-button slot="reference">查看</el-button>
              </el-popover>
<!--              {{data.newAddressid}}-->
            </el-descriptions-item>
            <el-descriptions-item label="描述图">{{data.returnImags}}</el-descriptions-item>
            <el-descriptions-item label="衣服模型参数JSON字符">{{data.newClothesjson}}</el-descriptions-item>
        </el-descriptions>
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      this.getPayTypeData();
      this.getRedoOrderStatusData();
    },
    methods: {
      getTitle(index){
        return "第"+(index+1)+"次重做信息";
      },
      /** 订单详情*/
      handleOrderDesc(row){
        console.log(row)
      ...
      handleOrderApproval(row){
        ...
        this.$confirm('确定发货?', '提示', {
          ...
          updateOrder(from).then(response=>{
            this.$message({
              type: 'success',
              message: '操作成功!'
            });
      ...
      //获取订单重做状态
      getRedoOrderStatusData(){
        getDicts("redo_status").then(response=>{
          this.redoOrderTypeList = response.data;
        })
      },
      //获取订单状态
      getOrderStatusData(){
