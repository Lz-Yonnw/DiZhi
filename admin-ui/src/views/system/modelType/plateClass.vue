    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:plateClass:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:plateClass:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:plateClass:remove']"
        >删除</el-button>
      </el-col>
<!--      <el-col :span="1.5">-->
<!--        <el-button-->
...
<!--          size="mini"-->
<!--          @click="handleExport"-->
<!--          v-hasPermi="['system:plateClass:export']"-->
<!--        >导出</el-button>-->
<!--      </el-col>-->
	  <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="plateClassList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
<!--      <el-table-column label="图片" align="center" prop="id" />-->
      <el-table-column label="名称" align="center" prop="name" >
        <template slot-scope="scope">
          <router-link :to="'/modelType/way/asList/' + scope.row.id" class="link-type">
...
<script>
import { listPlateClass, getPlateClass, delPlateClass, addPlateClass, updatePlateClass, exportPlateClass,getPlateClassAsList } from "@/api/system/plateClass";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      getPlateClassAsList(this.queryParams).then(response => {
    ...
    getPlateClassAsList(){
      this.loading = true;
      listPlateClass(this.queryParams).then(response => {
        this.plateClassList = response.rows;
        this.total = response.total;
        this.loading = false;
      });
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updatePlateClass(this.form).then(response => {
            ...
            addPlateClass(this.form).then(response => {
              if (response.code === 200) {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除板块大类-部位编号为"' + ids + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delPlateClass(ids);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有板块大类-部位数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return exportPlateClass(queryParams);
        }).then(response => {
