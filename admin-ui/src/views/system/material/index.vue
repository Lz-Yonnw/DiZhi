...
<!--                <i class="el-icon-edit" @click="openInnerDialogThree(data.classId)" style="border: none;font-size: 16px;cursor: pointer;margin-right: 10px;"></i>-->
    ...
    <el-dialog id="innerLogFour" title="添加部件" :visible.sync="openFour"  append-to-body>
      <el-form ref="componentForm" :model="componentForm" :rules="rulesFour" label-width="80px">
        ...
        <el-form-item label="模型上传" prop="modelUrl">
          <el-upload
            accept=".fbx"
            class="upload-demo"
            :headers="headers"
            action="/dev-api/common/model/upload"
            :file-list="fileModelList"
            :on-success="handleModelAvatarSuccess"
            >
            <el-button size="small" type="primary">点击上传</el-button>
            <div slot="tip" class="el-upload__tip">只能上传.fbx文件</div>
          </el-upload>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
...
export default {
  ...
  data() {
    ...
    return {
      ...
      options: {
        target: process.env.VUE_APP_BASE_API +'/common/uploadCategory',//SpringBoot后台接收文件夹数据的接口
  ...
  methods: {
    //模型文件上传成功
    handleModelAvatarSuccess(res,file){
      this.fileModelList = [];
      this.fileModelList.push(file)
      this.componentForm.modelUrl = res.fileName;
    },
    ...
    openInnerDialogTwo(classId){
      ...
      this.classId=classId;
      this.options.query.classId=classId;
      //
